...
> [`learner/m8-capstone/guide.md`](../../../learner/001-rust-foundation/m8-capstone/guide.md)。
...
输出是 [`Frame`](../../../experiments/001-rust-foundation/m8-capstone/src/parse.rs)：逻辑头 + 指向原缓冲的载荷视图。
失败是 [`ParseError`](../../../experiments/001-rust-foundation/m8-capstone/src/error.rs) 的四个变体，不是"出错了"。
...
- **一句话定义**：[`OwnedBytes`](../../../experiments/001-rust-foundation/m8-capstone/src/buffer.rs) 独占 `Vec<u8>`；
  [`PacketBuf<'a>`](../../../experiments/001-rust-foundation/m8-capstone/src/buffer.rs) 只保存 `&'a [u8]`，因此是 `Copy`。
...
- **一句话定义**：线上 `kind` 是 `u8`；逻辑上是 [`FrameKind`](../../../experiments/001-rust-foundation/m8-capstone/src/wire.rs) 枚举。
...
- **一句话定义**：[`ParseHeader`](../../../experiments/001-rust-foundation/m8-capstone/src/parse.rs) 对象安全，可 `&dyn ParseHeader`；
  [`parse_then`](../../../experiments/001-rust-foundation/m8-capstone/src/parse.rs) 对 `P: ParseHeader + ?Sized` 单态化。
...
- **一句话定义**：失败是 [`ParseError`](../../../experiments/001-rust-foundation/m8-capstone/src/error.rs) 四变体；
...
- **一句话定义**：[`FrameIter`](../../../experiments/001-rust-foundation/m8-capstone/src/iter.rs) 实现 `Iterator`；
  [`map_ok_payloads`](../../../experiments/001-rust-foundation/m8-capstone/src/iter.rs) 吃 `FnMut`；
  [`boxed_header_parser`](../../../experiments/001-rust-foundation/m8-capstone/src/iter.rs) 是 `Box<dyn ParseHeader>`。

- **底层机制**：空缓冲 `next` 返回 `None`，不先产一个 `Err`。失败后把 rest 置空，停止。
  `Box::new(HeaderParser)` **不会**分配：`HeaderParser` 是 ZST。所以装箱的是带 `u16`
  字段的 [`MagicParser`](../../../experiments/001-rust-foundation/m8-capstone/src/parse.rs)（`boxing_parser_allocates_once`）。
...
- **一句话定义**：[`ParseStats`](../../../experiments/001-rust-foundation/m8-capstone/src/stats.rs) 只含 `AtomicUsize`，
...
- **一句话定义**：[`raw`](../../../experiments/001-rust-foundation/m8-capstone/src/raw.rs) 对外全是安全函数；
...
- **一句话定义**：本 crate **不做**真实 C 调用。C-21 落在 [`wire.rs`](../../../experiments/001-rust-foundation/m8-capstone/src/wire.rs)
