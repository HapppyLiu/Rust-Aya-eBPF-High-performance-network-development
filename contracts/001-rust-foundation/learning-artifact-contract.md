...
下列 `learning/`、`feynman/`、`acceptance/`、`learner/` 路径从仓库根起算，
本 Feature 对应 `*/001-rust-foundation/`。从仓库根执行的枚举命令见 §E3。

**通用规则**：本契约中标注 REQUIRED 的小节缺失时，对应模块/能力 MUST NOT 被标记为完成。

---

## A. `learning/<feature>/mN-<module>/concept.md` —— 概念材料
...
## B. `learning/<feature>/mN-<module>/source-refs.md` —— 源码引用（FR-005）
...
## C. `feynman/<feature>/mN-<module>.md` —— Feynman 教学材料（FR-006 / Constitution IV）
...
## D. `acceptance/<feature>/criteria/cNN.md` —— 验收标准（FR-007 / Constitution V）
...
## E. `acceptance/<feature>/capability-matrix.md` —— 追踪链单一事实源（FR-013 / SC-011）
...
```bash
# 从仓库根目录执行。FEATURE 是 Feature 短名。
FEATURE=001-rust-foundation

# 步骤 1：枚举全部条目（文件级 + 小节级）
#   文件级：learning/ 与 feynman/ 与 learner/ 下该 Feature 的全部 .md（排除 _templates/ 与 README.md）
find "learning/$FEATURE" "feynman/$FEATURE" "learner/$FEATURE" -name '*.md' \
  -not -path '*/_templates/*' -not -name 'README.md' | sort > /tmp/rf-entries.txt

#   小节级：concept.md 中每个 "### C-xx" 小节、feynman 第 2/4 节中每个能力条目
grep -rhoE '^### (C-[0-9]{2})' learning/"$FEATURE"/*/concept.md | sort -u > /tmp/rf-sections.txt

# 步骤 2：枚举矩阵中登记的 C-ID
grep -oE '^\| (C-[0-9]{2})' "acceptance/$FEATURE/capability-matrix.md" \
...
#   路径形如 learning/001-rust-foundation/mN-... 、feynman/001-rust-foundation/mN-*.md
#   mN MUST 出现在矩阵 Module 列
cut -d/ -f3 /tmp/rf-entries.txt | grep -oE '^m[1-8]' | sort -u
grep -oE '\| m[1-8] \|' "acceptance/$FEATURE/capability-matrix.md" | grep -oE 'm[1-8]' | sort -u
```
