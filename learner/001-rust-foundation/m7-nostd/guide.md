...
|------|-----|---------|---------|-----------|
| C-22 | `c22_nostd` 默认构建 | `cd experiments/001-rust-foundation/m7-nostd && cargo build` 的退出码；构建脚本是否拒绝非裸机 target | **构建是否成功**（成功与否是断言对象；不要断言诊断全文） | 填预测表 #0 |
...
- **Hint 3（拆解）**：`dealloc` 若什么都不做，生命周期那一条怎么写才不算撒谎？
  写完用 `cd experiments/001-rust-foundation/m7-nostd && cargo clippy -- -D warnings` 做机械兜底。
